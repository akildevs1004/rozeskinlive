		...
		<?php elseif( $format == 'video' || $format == 'audio' ) : ?>
			...
			<div class="entry-thumb single-thumb">
				<a class="post-thumbnail" href="<?php esc_url(the_permalink()); ?>" title="<?php echo the_title_attribute(); ?>">
					<?php the_post_thumbnail( 'aenyo-full-width', array( 'alt' => get_the_title() ) );	?>      
				</a> 
			</div>
			<?php } ?>
						...
						<div class="slick-carousel" data-columns4="1" data-columns3="1" data-columns2="1" data-columns1="1" data-columns="1" data-nav="true">
								<?php
									if($ids){
										...
										foreach ( $ids as $i => $id ){ ?>
											<div class="item">
													<?php echo wp_get_attachment_image($id, 'aenyo-full-width'); ?>
											</div>
										<?php }	
									}
								?>
		...
		<div class="post-content">
			...
			<?php
        		if (aenyo_get_config('blog-excerpt')) {
                    echo aenyo_get_excerpt( aenyo_get_config('list-blog-excerpt-length',70), false);
                }
			?>
    	</div>	
    </article><!-- #post-## -->
